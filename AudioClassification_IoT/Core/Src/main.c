...
char ssid[] = "Galaxy A35 5G B9A7";
char passphrase[] = "firstclass";
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    {
        mic_x = (float)HAL_ADC_GetValue(&hadc1);
        adc_ready = 1;
    }
}
/* USER CODE END 0 */
...
int main(void)
{
  	...
  	if (error_code != NEAI_OK)
 {
  ...
  MX_SPI3_Init();
  /* USER CODE BEGIN 2 */
  // Initialize Wi-Fi and MQTT functionalities in a clean environment first
  wifi_init();
  mqtt_init();

  // Start ADC and Timer data acquisition after Wi-Fi and MQTT are successfully initialized
...
void Inference()
{
    ...
    if(id_class_t0 == id_class_tn)
    {
        const char *class_name = neai_get_class_name(id_class_t0);
        if (class_name == NULL) {
            class_name = "unknown";
        }
        printf("Detected Class: %s\r\n", class_name);
        fflush(stdout);
        if (strcmp(class_name, "Baby Cry") == 0) {
...
static void mqtt_init(void) {
    strcpy(hwifi.mqtt.publishTopic, "menon1419/feeds/audio-classification");
    // If you don't want to subscribe to any topic, you can skip this line or set it to an empty string.
    strcpy(hwifi.mqtt.subscribeTopic, "menon1419/feeds/audio-classification");
    hwifi.mqtt.securityMode = WIFI_MQTT_SECURITY_USER_PW;
    strcpy(hwifi.mqtt.userName, "YOUR_ADAFRUIT_USERNAME");
    strcpy(hwifi.mqtt.password, "YOUR_ADAFRUIT_IO_KEY");
...
void mqtt_publish(const char* message) {
    ...
    // Temporarily pause high-frequency ADC interrupts to prevent SPI corruption
