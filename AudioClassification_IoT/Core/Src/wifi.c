...
WIFI_StatusTypeDef WIFI_SPI_Transmit(WIFI_HandleTypeDef* hwifi, char* buffer, uint16_t size){
	...
	if ( !(size % 2) ) { // If buffer had an odd amount of bytes, append a filler char to bTx
		uint16_t len = strlen(bTx);
		bTx[len] = (char) WIFI_TX_PADDING;
		bTx[len + 1] = '\0';
	}
...
WIFI_StatusTypeDef WIFI_WebServerListen(WIFI_HandleTypeDef* hwifi){
	...
	// Send response
	msgLength = snprintf(wifiTxBuffer, WIFI_TX_BUFFER_SIZE, "S3=%u\r", (unsigned int)strlen(wifiRxBuffer));
	strncat(wifiTxBuffer, wifiRxBuffer, WIFI_TX_BUFFER_SIZE - msgLength - 1);
	msgLength = strlen(wifiTxBuffer);
	WIFI_SendATCommand(hwifi, wifiTxBuffer, msgLength+1, wifiRxBuffer, WIFI_RX_BUFFER_SIZE);
